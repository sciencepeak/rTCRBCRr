...
### Read raw data files into a list of data frames
...
### Execution has two types: loose and tight

#### Loose execution

##### Tidy up the clonotype dataframes

The cleaning function completely\_tidy\_up\_clonotype consists of four
steps, namely four functions:

1.  format\_clonotype\_to\_immunarch\_style
2.  remove\_nonproductive\_CDR3aa
3.  annotate\_chain\_name\_and\_cell\_type
4.  merge\_convergent\_clonotype

##### Calculate and merge repertoire metrics for each sample in the list

This step consists of two functions.

1.  compute\_repertoire\_metrics\_by\_chain\_name
2.  combine\_all\_sample\_repertoire\_metrics
...
``` r
divergent_clonotype_dataframe_list <- raw_clonotype_dataframe_list %>%
    lapply(., completely_tidy_up_clonotype, adopted_clonotyping_tool = "trust")
...
### Separate the all\_sample\_all\_chain\_all\_metrics\_wide\_dataframe by individual metrics.
...
follows, where ![p\_i](https://latex.codecogs.com/png.latex?p_i "p_i")
is the frequency of
...
![
Shannon\\ entropy=-\\sum\_{i=1}^{N}{p\_i\\log\_2{\\left(p\_i\\right)}}
...
![
Pielou\\prime s\\ eveness\\ =\\ \\frac{Shannon\\ entropy}{\\log\_2{N}}
